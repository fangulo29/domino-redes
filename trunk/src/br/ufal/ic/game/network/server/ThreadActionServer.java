...
public class ThreadActionServer implements Runnable {
	...
	// será entregue a mensagem para cada jogador na lista de conectados no
	// servidor
	...
	while (true) {
		...
		// adicionando na lista de nomes dos jogadores conectados no
		// servidor
		server.addNameInListNameOfConnectedPlayers(nomeJogadorAdicionado);

		// lista de nomes dos players conectados no servidor
		List<String> namesOfConnectedPlayersInServer = server
			.getNamesOfConnectedPlayers();

		// setando a lista dos nomes dos jogadores (serializada) em
		// mensagem, que será entregue a todos
		mensagemRecebida.setNomesJogadoresConectadosGSON(gson
			.toJson(namesOfConnectedPlayersInServer));

		System.out.println("ENCAMINHANDO PARA TODOS : "
			+ namesOfConnectedPlayersInServer);
		...
		/*
		 * para cada jogador conectado no servidor (sockets), adicionar
		 * um player para referência no game
		 */
		...
		if (listaJogadoresDomino.size() >= 2
		    ...
		    Map<Integer, List<DominoPiece>> mapPlayesCurrentPieces = new HashMap<Integer, List<DominoPiece>>();
		    ...
		    // atualizando o map de players e suas respectivas peças NO
		    // SERVIDOR
